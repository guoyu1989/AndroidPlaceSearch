...
import com.jiraiyeah.androidplacesearch.utils.PlaceDetailsJSONParser;
import com.jiraiyeah.androidplacesearch.utils.PlaceJSONParser;
    ...
    switch(mUriMatcher.match(uri)){
      case SEARCH:
        ...
        try {
          ...
          list = PlaceJSONParser.parse(new JSONObject(jsonString));
      ...
      case SUGGESTIONS :
        ...
        try {
          ...
          for(int i=0;i<list.size();i++){
            ...
            // Adding place details to cursor
            mCursor.addRow(new String[] {
                Integer.toString(i), hMap.get("description"), hMap.get("reference") });
          }
        } catch (JSONException e) {
          e.printStackTrace();
        }
